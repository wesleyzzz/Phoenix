...
#include "ADPrecipitation_liquid.h"
#include <cmath>

registerADMooseObject("PhoenixApp", ADPrecipitation_liquid);

defineADValidParams(ADPrecipitation_liquid,
                    ADKernel,
...
ADPrecipitation_liquid<compute_stage>::ADPrecipitation_liquid(const InputParameters & parameters) :
...
template <ComputeStage compute_stage>
ADReal
ADPrecipitation_liquid<compute_stage>::computeQpResidual()
{
  ADReal r = 0.0;
  ADReal prefactor = _scale_factor[_qp] * _sign;

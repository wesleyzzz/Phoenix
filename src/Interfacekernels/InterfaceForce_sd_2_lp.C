// Describe s_d --> l_p, coupled neighbor _coupled_s_p

#include "InterfaceForce_sd_2_lp.h"
#include <cmath>

registerMooseObject("PhoenixApp", InterfaceForce_sd_2_lp);
...
InterfaceForce_sd_2_lp::InterfaceForce_sd_2_lp(const InputParameters & parameters) :
...
InterfaceForce_sd_2_lp::getPostprocessorValue(const std::string & name)
{
...
const PostprocessorValue &
InterfaceForce_sd_2_lp::getPostprocessorValueByName(const PostprocessorName & name)
{
...
Real
InterfaceForce_sd_2_lp::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0.0;
  Real Diff_eff = _diffusivity_in_solid_sd[_qp];
  Real Force_eff = _driving_rate[_qp] * (_neighbor_value[_qp] - _solubility_in_solid[_qp]);

  if (_u[_qp] < _stop_threshold && _coupled_s_p[_qp] < _stop_threshold)
  {
...
Real
InterfaceForce_sd_2_lp::computeQpJacobian(Moose::DGJacobianType type)
{
  Real jac = 0.0;

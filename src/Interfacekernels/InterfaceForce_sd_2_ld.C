// Describe s_d --> l_d, coupled neighbor _coupled_s_p

#include "InterfaceForce_sd_2_ld.h"
#include <cmath>

registerMooseObject("PhoenixApp", InterfaceForce_sd_2_ld);
...
InterfaceForce_sd_2_ld::InterfaceForce_sd_2_ld(const InputParameters & parameters) :
...
InterfaceForce_sd_2_ld::getPostprocessorValue(const std::string & name)
{
...
const PostprocessorValue &
InterfaceForce_sd_2_ld::getPostprocessorValueByName(const PostprocessorName & name)
{
...
Real
InterfaceForce_sd_2_ld::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0.0;
  ...
  if (_coupled_l_p[_qp] < _stop_threshold && _coupled_s_p[_qp]< _stop_threshold)
  {
...
Real
InterfaceForce_sd_2_ld::computeQpJacobian(Moose::DGJacobianType type)
{
  Real jac = 0.0;
  Real Diff_eff = _diffusivity_in_solid[_qp];
